...
public class UserService {
	...
	public User selectByOpenidNoEx(String openid) {
		User user = jedis.get(Constant.JedisNames.UserOpenid + openid, User.class);
		if(user == null) {
			user = userMapper.selectByOpenid(openid);
			if(user == null) {
	...
	public User selectOne(long id) {
		User user = jedis.get(Constant.JedisNames.UserId + id, User.class);
		...
		if(user != null) {
			jedis.set(Constant.JedisNames.UserId + id, JSON.toJSONString(user), Constant.JedisNames.OneDay);
		}
