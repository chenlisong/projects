...
public class CommonUtil {
	...
	public static <T> List<T> entitySkip(List<? extends Object> list,String fieldName,Class<? extends Object> t, Object... skips){
		List<T> result = new ArrayList<T>();
		List skipList = Arrays.asList(skips);
		...
		for(Object o : list){
			...
			String getMethod = "get"+fieldName.substring(0,1).toUpperCase()+fieldName.substring(1);
			try {
				T tObject = (T) o.getClass().getDeclaredMethod(getMethod).invoke(o);

				if (!skipList.contains(tObject)) {
					result.add(tObject);
				}
			} catch (Exception e) {
				throw new RuntimeException("CommonUtil entity error ."+e);
	...
	public static void setDefaultValue(Object obj){
		try{
			...
			for(Field field : fields){
				...
				//"id"跳过
				if(!"id".equals(fieldName) || !"createTime".equals(fieldName) || fieldName == null || fieldName.length() <= 0){
